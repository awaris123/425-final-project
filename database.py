...
def register_new_model(model_number, cost, sale_price, lead_time=timedelta(), category=0, quantity=0):
    ...
    con.cursor().execute('SELECT InventoryID FROM Inventory WHERE ModelNumber=?', (model_number,))
    row = con.cursor().fetchone()
    inventory_id = row[0]

    con.cursor().execute('INSERT INTO Model(ModelNumber, InventoryID, SalePrice) VALUES(?,?,?)', (model_number, inventory_id, sale_price))
    con.commit()
    con.close()
